...
def blockbed_list(request):
    print("inside reservation list")
    print("Hospital name is -----> ",request.user.username)
    user_name=request.user.username
...
def bed_availability(request):
    ...
    dict ={}
    for each in h:
        hos = Hospital.objects.get(hospital_id=str(each))
        dict[str(each)]=[hos.hospital_name,hos.address,hos.phone_no]
...
#     print('print pk',pk)
#     print(request.method)
#     # blockbed=get_object_or_404(BlockBed,pk=str(pk))
#     return render(request,'eBedTrack/block_bed.html')
# #     if  (request.method == 'POST'):
# #         print("yes post method",request.method)
# #         form = BlockForm(request.POST,instance=blockbed)
# #         if(form.is_valid()):
# #             print('yes form is valid')
# #             blockbed = form.save(commit=False)
# #             blockbed.save()
# #             blockbed = BlockBed.objects.filter(created_date=timezone.now())
# #             print('data saved')
# #             blockbed=BlockBed.objects.filter(hospital_id=pk)
...
def block_bed(request,pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            blockbed = form.save(commit=False)
            blockbed.hospital_id_id=str(pk)
            blockbed.created_date=timezone.now()
            blockbed.save()
            return render(request,'eBedTrack/thanks.html')
        else:
            form = BlockForm()
            print('form is invalid')
            return render(request, 'eBedTrack/block_bed.html',{'form': form})
    else:
        form=BlockForm()
        return render(request, 'eBedTrack/block_bed.html',{'form': form})
